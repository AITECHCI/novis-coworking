...
export default function Description({ space }: { space: any }) {
    return (
        <section className="relative max-w-[1400px] mx-auto bg-gray-900 container px-2 md:px-8 py-20 lg:py-32">
            <div className="relative gap-8 items-stretch py-8 px-4 mx-auto max-w-screen-xl xl:gap-16 md:grid md:grid-cols-2 sm:py-16 lg:px-6">
                                        ...
                                        <table key={key} className="w-full text-sm text-left rtl:text-right text-gray-200">
                                            ...
                                            <tbody>
                                                {option.items.map((item: any, k: number) => (
                                                    <tr key={k} className="border-b list-disc odd:bg-gray-900 even:bg-gray-800">
                                                        <td className="py-1">
                                                            <li>{item}</li>
                                                        </td>
                                                    </tr>
                            ...
                            <div className="py-6">
                                ...
                                <ul className="space-y-4 text-gray-200 list-decimal list-inside">
                                    {space.tarifs.map((tarif: any, key: number) => (
                                        <li key={key} className="text-sm md:text-md">
                                            {tarif.name}
                ...
                <Image 
                    ...
                    src={space.images.map((image: any) => image.src)[0]} 
                    alt="dashboard image" 
                ...
                <>
                    ...
                    <div className="relative grid py-8 lg:grid-cols-2 justify-center gap-8 mb-8 lg:mb-16">
                        {space.items.map((subSpace: any, key: number) => (
                            <Card key={key} className={cn("w-full mx-4 relative max-w-[320px] sm:max-w-lg md:max-w-2xl")}>
            ...
            <AnimatedGridPattern
                ...
                duration={2}
                repeatDelay={1}
                className="[mask-image:radial-gradient(500px_circle_at_center,white,transparent)] inset-x-0 w-full h-[150%]"
            />
